...
import End from "../Parts_of_components/for-card/end";
import Header from "../Parts_of_components/for-card/head";
import Middle from "../Parts_of_components/for-card/middle";
...
const Card = () =>{
  ...
  return(
    <>
     <button onClick={see} className={isvisible? "bg-red-500 border rounded m-3 p-1":"p-1 bg-green-500 border rounded m-3"}>
        {isvisible ? "OFF":"ON"}
      </button>
    <div className={isvisible ? "flex-col  justify-center": "flex flex-wrap justify-center "}>
        {
          jobOpenings.map((detail, index)=>(
          <div key={index}>
            <div className=" bg-white w-[325px] h-[400px] m-10 px-2.5 py-3 rounded-4xl ">
              <Header logo={detail.brandLogo} />
              <Middle companyName={detail.companyName} datePosted={detail.datePosted} tag1={detail.tag1} tag2={detail.tag2}/>
              <End pay={detail.pay} location={detail.location}/>
            </div>
          </div>
          ))
        }
